...
export default function Navbar({ handleSave }) {
    ...
    useEffect(() => {
        if (showPreviewModal) {
            try {
                const stored = JSON.parse(localStorage.getItem("currentBlocks"));
                if (Array.isArray(stored)) setPreviewBlocks(stored);
            } catch (err) {
                console.error("Không thể load blocks từ localStorage");
            }
        }
    }, [showPreviewModal]);
    ...
    return (
        <div className="border-b shadow-sm bg-white">
            ...
            {/* Preview Modal */}
            {showPreviewModal && (
                <PreviewModal open={showPreviewModal} onClose={() => setShowPreviewModal(false)} device={device} />
            )}
        </div>
    );
}
